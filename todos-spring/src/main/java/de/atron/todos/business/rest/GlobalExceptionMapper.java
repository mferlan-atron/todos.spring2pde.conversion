...
/**
 * ExceptionMapper for all uncaught exceptions.
 * Exception message is transmitted in header
 */
@Provider
public class GlobalExceptionMapper implements ExceptionMapper<Exception> {
	final static Logger logger = Logger.getLogger(GlobalExceptionMapper.class);
    @Override
    public Response toResponse(Exception exception) {
    	logger.error("rest-exception", exception);
        return Response.status(Response.Status.BAD_REQUEST).header("error-message", exception.getMessage()).build();
    }
